...
from Model_Raw_w1 import DUVModel

os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
os.environ["CUDA_VISIBLE_DEVICES"] = '0'
os.environ["HDF5_USE_FILE_LOCKING"] = "FALSE"
BACKBONE = 'Model-Raw-w1'
...
def main(csv, ckpt):
    ...
    train_dataset = DUVDataset(root_dir, train_df)
    ...
    val_dataset = DUVDataset(root_dir, val_df)
    ...
    summary_str = str(summary(model, input_size=(batch_size, 3, 439, 439), verbose=0))
...
# ===================================================================================================================
        ...
        if epoch == 19:
            total = sum(time_consumption)
            average = total / len(time_consumption)
            logger.info('Average spent {} seconds / per epoch'.format(average))
            logger.info('================================================')
...
class DUVDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(join(DEST_DEV_REMOTE, 'tsv2nd_{}.{}'.format(sn, HDF5_EXT_RAW)), "r") as f:
            Ex = f['Ex'][self.start_freq]
            Ey = f['Ey'][self.start_freq]
            Ez = f['Ez'][self.start_freq]

        E = np.concatenate((np.expand_dims(Ex, axis=0), np.expand_dims(Ey, axis=0), np.expand_dims(Ez, axis=0)), axis=0)
