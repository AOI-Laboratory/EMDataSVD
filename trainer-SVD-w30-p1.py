...
from Model_SVD_w30_p1 import DUVModel

os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
os.environ["CUDA_VISIBLE_DEVICES"] = '0'
BACKBONE = 'Model-SVD-w30-p1'
...
def main(csv, ckpt):
    ...
    summary_str = str(summary(model, input_size=(batch_size, 1, 439, 180), verbose=0))
    ...
    for epoch in range(num_epochs):
        ...
        val_spent_time = 0.0
        for ti, (inputs, targets) in enumerate(train_dataloader):
            start_time = time.time()
            inputs = inputs.to(device)
            targets = targets.to(device)
            pred_ocd = model(inputs)
...
# ===================================================================================================================
        ...
        model.eval()
        with torch.no_grad():
            for vi, (inputs, targets) in enumerate(val_dataloader):
                start_time = time.time()
                inputs =inputs.to(device)
                targets = targets.to(device)
                pred_ocd = model(inputs)
...
class DUVDataset(Dataset):
    ...
    def __transform__(self, small_matrix):
        ...
        return np.concatenate((Us, V), axis=-1)
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(join(self.dest_dev, 'tsv2nd_{}.{}'.format(sn, HDF5_EXT_SVD)), "r") as f:
            Ex = np.expand_dims(f['Ex'][self.start_freq::self.step_freq, :, 0], axis=-1)
            Ey = np.expand_dims(f['Ey'][self.start_freq::self.step_freq, :, 0], axis=-1)
            Ez = np.expand_dims(f['Ez'][self.start_freq::self.step_freq, :, 0], axis=-1)

        for i in range(Ex.shape[0]):
            ex = self.__transform__(Ex[i])
            ey = self.__transform__(Ey[i])
            ez = self.__transform__(Ez[i])

            if 'multi_Ex' not in locals():
                multi_Ex = ex
            else:
                multi_Ex = np.concatenate((multi_Ex, ex), axis=-1)

            if 'multi_Ey' not in locals():
                multi_Ey = ey
            else:
                multi_Ey = np.concatenate((multi_Ey, ey), axis=-1)

            if 'multi_Ez' not in locals():
                multi_Ez = ez
            else:
                multi_Ez = np.concatenate((multi_Ez, ez), axis=-1)

        E = np.concatenate((multi_Ex, multi_Ey, multi_Ez), axis=-1)
