...
from Model_Raw_w5 import DUVModel

# os.environ["PYTORCH_NO_CUDA_MEMORY_CACHING"] = "1"
os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
os.environ["CUDA_VISIBLE_DEVICES"] = '0'
os.environ["HDF5_USE_FILE_LOCKING"] = "FALSE"
BACKBONE = 'Model-Raw-w5'
...
def main(csv, ckpt):
    ...
    summary_str = str(summary(model, input_size=[(batch_size, 5, 439, 439), (batch_size, 5, 439, 439), (batch_size, 5, 439, 439)], verbose=0))
    ...
    for epoch in range(num_epochs):
        ...
        val_spent_time = 0.0
        for ti, ((Ex, Ey, Ez), targets) in enumerate(train_dataloader):
            start_time = time.time()
            Ex = Ex.to(device)
            Ey = Ey.to(device)
            Ez = Ez.to(device)
            targets = targets.to(device)
            pred_ocd = model(Ex, Ey, Ez)
...
# ===================================================================================================================
        ...
        model.eval()
        with torch.no_grad():
            for vi, ((Ex, Ey, Ez), targets) in enumerate(val_dataloader):
                start_time = time.time()
                Ex = Ex.to(device)
                Ey = Ey.to(device)
                Ez = Ez.to(device)
...
class DUVDataset(Dataset):
    def __init__(self, root_dir, data, type):
        ...
        if type == 0:
            self.dest_dev = join(DEST_DEV_REMOTE, 'E_Train')
        else:
            self.dest_dev = join(DEST_DEV_REMOTE, 'E_Val')
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(join(self.dest_dev, 'tsv2nd_{}.{}'.format(sn, HDF5_EXT_RAW)), "r") as f:
            Ex = f['Ex'][self.start_freq::self.step_freq]
            Ey = f['Ey'][self.start_freq::self.step_freq]
            Ez = f['Ez'][self.start_freq::self.step_freq]

        ocd = torch.tensor(ocd, dtype=torch.float)
        Ex = torch.tensor(Ex, dtype=torch.float)
        Ey = torch.tensor(Ey, dtype=torch.float)
        Ez = torch.tensor(Ez, dtype=torch.float)
